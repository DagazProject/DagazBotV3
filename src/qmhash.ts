...
export class QmContext {
    ...
    constructor(public readonly name: string, public readonly loc: number, public ix) {}
}
...
export function load(name: string): QmContext {
    try {
        let ix = null;
        for (let i = 0; i < hash.length; i++) {
             if (hash[i].name == name) {
                 hash[i].date = new Date();
                 return new QmContext(hash[i].name, hash[i].loc, i);
             }
        ...
        return new QmContext(name, loc, ix);
    } catch (error) {
       console.error(error);
    }
}
